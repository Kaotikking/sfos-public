...
def test_converge_accepts_only_governed_source_roads_and_reuses_atomic_transaction():
 ...
 assert 'base-road-transaction.py" install' in s
 assert 'OUTPOST_BOOTED_AND_HOST_VERIFIED' in (R/'installer/base-road-transaction.py').read_text()
 assert 'ROLLBACK_BASE_REQUIRED' in s and 'IMMUTABLE_INPUT_PLAN_REQUIRED' in s
 assert "stat -c '%a:%u:%g'" in s and '600:0:0' in s
 assert s.index('outpost.host_witness_runner') < s.index('base-road-transaction.py" install')
 assert 'systemctl enable --now serein-outpost.target' not in s
 assert not any(x in s for x in ('VM4010','PVE_REST_QGA','parted ','mkfs','grub-install','apt-get install','shutdown -','reboot\n'))
